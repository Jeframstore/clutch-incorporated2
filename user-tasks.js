...
function loadCurrentTask() {
    ...
    dailyTasks.forEach(task => {
        ...
        if (taskEndTime < currentTime) {
            pastTasks.push({ ...task, isAvailable: false, timePeriod: 'past' });
        ...
        // Categorize as present tasks (within task time window)
            ...
            presentTasks.push({ ...task, isAvailable: true, timePeriod: 'present' });
        ...
        // Categorize as next task (start time is in future, only one)
            ...
            nextTasks.push({ ...task, isAvailable: true, timePeriod: 'next' });
            hasNextTask = true;
            console.log(`Task ${task.taskNumber}: NEXT`);
        }
    });
    
    console.log('Past tasks:', pastTasks.map(t => ({ number: t.taskNumber })));
    console.log('Present tasks:', presentTasks.map(t => ({ number: t.taskNumber })));
    console.log('Next tasks:', nextTasks.map(t => ({ number: t.taskNumber })));

    // Display tasks in their respective sections
    displayTasksInSection('pastTasks', pastTasks, 'past');
    displayTasksInSection('presentTask', presentTasks, 'present');
    displayTasksInSection('nextTask', nextTasks, 'next');

    // Check if all tasks are expired (end of day)
    if (pastTasks.length > 0 && presentTasks.length === 0 && nextTasks.length === 0) {
        document.getElementById('presentTask').innerHTML = `
            <div class="end-of-day-message">
                <i class="fas fa-check-circle"></i>
                <h3>Thank you for today's tasks</h3>
                <p>Rest well and wait for tomorrow</p>
            </div>
        `;
        document.getElementById('nextTask').innerHTML = '<div style="text-align: center; color: #64748b; padding: 2rem;">No upcoming tasks today.</div>';
    }
}
